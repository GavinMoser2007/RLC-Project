...
## What Did I learn? (Gavin)
Although I already had a solid understanding of the fundamentals of RLC circuits from taking Circuits I, this project expanded my knowledge by showing how differential equations can be used to model the relationships between electrical components over time. Instead of just solving equations, I learned how the mathematical model directly predicts the charging and discharging behavior of a real circuit, and how that behavior changes depending on whether the circuit is underdamped, overdamped, critically damped, or undamped. I also gained a much better understanding of how a capacitor and inductor behave when connected in parallel. Since they share the same node, their voltages are tied together, causing them to approach their DC steady-state conditions at roughly the same time, around five time constants. This was a big step in my circuit learning. Going into this project, I made the wrong assumption about using the capacitor and inductor time-constant equations (RC and L/R). However, learning more about the parallel nodes taught me that this could only be applicable when used in purely an RC or RL circuit. This did not apply because the capacitor and inductor's charging and discharging were affecting one another. Seeing this in both the mathematical model and the physical circuit helped deepen my understanding of how parallel RLC circuits actually operate.

Another thing I learned was how useful an Arduino UNO can be beyond simply sending or receiving voltage signals. By using its analog input, it effectively acted as a voltmeter that continuously measured the capacitor's voltage over time, allowing us to collect experimental data and graph it in Python. Comparing those measurements to the differential equation model showed me how mathematics, programming, and electrical engineering all work together to accurately predict and analyze the behavior of real circuits, while also highlighting the small differences caused by non-ideal components and experimental error.

Finally, I learned the importance of protecting a microcontroller when measuring voltages in an RLC circuit. Most microcontrollers have input voltage limits determined by their supply voltage (typically 3.3 V or 5 V), and applying voltages outside the allowable range can damage the device. In an underdamped RLC circuit, the capacitor voltage may oscillate and can become negative or exceed the supply voltage, potentially driving the microcontroller input beyond its safe operating limits. To protect the microcontroller, a series resistor can be used to limit the input current, while clamping diodes or other protection circuitry can prevent the input voltage from exceeding the allowable range.
